...
namespace Tratamento_Excecoes
{
    class Teoria
    {
        static void Main(string[] args)
        {
            ...
            catch (DivideByZeroException)         //se for dividido por zero, erro
            {
                Console.WriteLine("Division by zero is not allowed"); //passando a mensagem de erro de forma direta
            }
            catch (FormatException e) // "e" nome da mensagem //pegando erro caso seja colocado uma string no lugar de número
            {
                Console.WriteLine("Format error! " + e.Message);  // "Format error!" + "Division by zero is not allowed"
            }
            #endregion
